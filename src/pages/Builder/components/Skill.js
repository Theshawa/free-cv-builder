...
const SkillLevel = ({setLevel,level}) => {
...
const Skill = ({ data, save, remove }) => {
    ...
    return (
        <div className=" b bg-light bg-opacity-30 flex flex-col w-full">
            <button onClick={() => setActive(true)} className="p-8 flex items-center justify-between text-xl gap-4"><span>{(name.current && name.current.value) || `Skill ${data.id}`}</span> {!active && <span>✏️</span>}</button>
            <form onSubmit={saveData} className={`${active ? 'flex' : 'hidden'} flex-col gap-8 px-8 pb-8`} noValidate>
                <div className="flex flex-col gap-8">
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <SkillLevel level={level} setLevel={(index)=>setLevel(index)}/>
                    </div>
